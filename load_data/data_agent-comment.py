...
def get_response(model_name, prompt):
    ...
    logger.info("GPT loading {}...".format(model_name))
    ...
    if usage:
        logger.info(f"[LLM TOKENS] Prompt Tokens: {usage.prompt_tokens} | Completion Tokens: {usage.completion_tokens} | Total Tokens: {usage.total_tokens}")
    else:
        logger.warning("[LLM TOKENS] Usage data not returned by the API.")
...
def get_label(input_string, answer):
    ...
    planing = get_response("gpt-4o", prompt_template)   # Sends a prompt to the OpenAI API and retrieves the response.
    ...
    query_response = extract_knowledge_based(planing)
    logger.info(f">>> Extracted Queries: {query_response}")
    
    logger.info(">>> STEP 3: Calling NER Agent to fetch factual knowledge...")
    formatted_query_response = NER_agent(query_response)
    ...
    for item in parsed_dict.keys():
        ...
        planing = planing.replace(item, item + " [rag]" + parsed_dict[item]) 

    # 4. Extract and format the finalized steps into a list
    logger.info(">>> STEP 6: Formatting final labeled content...")
    final_list = extract_labeled_content_as_list(planing)
